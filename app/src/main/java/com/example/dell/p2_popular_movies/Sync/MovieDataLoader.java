        ...
        try{
            ...
            for(int i = 0; i < moviesArray.length(); i++) {
                ...
                if (!utility.isMovieIdFavorite(favoriteMovieIds, movieId))
                {
                    ...
                    movieValues.put(MoviesContract.MovieEntry.COLUMN_BACK_DROP_PATH, backdropPath);
                    movieValues.put(MoviesContract.MovieEntry.COLUMN_ORIGINAL_LANGUAGE, originalLanguage);
                    ...
                    movieValues.put(MoviesContract.MovieEntry.COLUMN_IS_VIDEO, isVideo);
                    ...
                    movieValues.put(MoviesContract.MovieEntry.COLUMN_DATE, mDayTime.setJulianDay(mJulianStartDay));
        ...
        private int getTrailersDataFromJson(String moviesJsonStr)
            ...
            try {
                ...
                for (int i = 0; i < moviesArray.length(); i++) {
                    ...
                    trailerValues.put(MoviesContract.MovieTrailerEntry.COLUMN_TRAILER_ID, trailerId);
                    trailerValues.put(MoviesContract.MovieTrailerEntry.COLUMN_ISO_369_1, ISO_639_1);
                    ...
                    trailerValues.put(MoviesContract.MovieTrailerEntry.COLUMN_SITE, site);
                    trailerValues.put(MoviesContract.MovieTrailerEntry.COLUMN_SIZE, size);
                    trailerValues.put(MoviesContract.MovieTrailerEntry.COLUMN_TYPE, type);
                    trailerValues.put(MoviesContract.MovieEntry.COLUMN_DATE, mDayTime.setJulianDay(mJulianStartDay));
                ...
                if (cVVector.size() > 0) {
                    ContentValues[] cvArray = new ContentValues[cVVector.size()];
                    cVVector.toArray(cvArray);
                    inserted = mContext.getContentResolver().bulkInsert(MoviesContract.MovieTrailerEntry.CONTENT_URI, cvArray);
                }
                setMovieStatus(mContext, MOVIE_STATUS_OK);
            } catch (JSONException e) {
                Log.e(LOG_TAG, e.getMessage(), e);
...
//            http://api.themoviedb.org/3/movie/135397/reviews?api_key=28edec251247e1d7328ab3ec7f483cd0
        ...
        private int getReviewsDataFromJson(String moviesJsonStr)
            ...
            try {
                ...
                for (int i = 0; i < moviesArray.length(); i++) {
                    ...
                    reviewValues.put(MoviesContract.MovieReviewEntry.COLUMN_REVIEW_ID, reviewId);
                    ...
                    reviewValues.put(MoviesContract.MovieReviewEntry.COLUMN_DATE, mDayTime.setJulianDay(mJulianStartDay));
                ...
                if (cVVector.size() > 0) {
                    ContentValues[] cvArray = new ContentValues[cVVector.size()];
                    cVVector.toArray(cvArray);
                    inserted = mContext.getContentResolver().bulkInsert(MoviesContract.MovieReviewEntry.CONTENT_URI, cvArray);
                }
                setMovieStatus(mContext, MOVIE_STATUS_OK);
            } catch (JSONException e) {
                Log.e(LOG_TAG, e.getMessage(), e);
    ...
    private void deleteOldData() {
        ...
        if (favoriteMovieIds != null && favoriteMovieIds.length > 0) {
            ...
            criteriaString = MoviesContract.MovieEntry.COLUMN_MOVIE_ID + " NOT IN (" + favoriteMovieIdsString + ")";
        }

        // delete old data so we don't build up an endless history
        int deleted = mContext.getContentResolver().delete(MoviesContract.MovieReviewEntry.CONTENT_URI,
                criteriaString,
                null);
        ...
        // delete old data so we don't build up an endless history
        deleted = mContext.getContentResolver().delete(MoviesContract.MovieTrailerEntry.CONTENT_URI,
                criteriaString,
                null);
